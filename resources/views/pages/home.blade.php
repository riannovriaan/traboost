@extends('layouts.app')

@section('title')
    Traboost
@endsection

@section('content')
    <!-- Header -->
    <header class="text-center">
        <h1>
            Your Travel Booster
            <br>
            To See Beautiful World
        </h1>
        <p class="mt-3">
            You will discover many beautiful place
            <br>
            you haven't seen before
        </p>
        <a href="#popular" class="btn btn-get-started px-4 mt-4">
            GET STARTED!
        </a>
    </header>


    <main>
        <div class="container">
            <section class="section-stats row justify-content-center" id="stats">
                <div class="col-3 col-md2 stats-detail cente">
                    <h2>
                        20K
                    </h2>
                    <p>Members</p>
                </div>
                <div class="col-3 col-md2 stats-detail">
                    <h2>
                        12
                    </h2>
                    <p>Countries</p>
                </div>
                <div class="col-3 col-md2 stats-detail">
                    <h2>
                        3K
                    </h2>
                    <p>Hotels</p>
                </div>
                <div class="col-3 col-md2 stats-detail">
                    <h2>
                        72
                    </h2>
                    <p>Partners</p>
                </div>
            </section>
        </div>
        <section class="section-popular" id="popular">
            <div class="container">
                <div class="row">
                    <div class="col text-center section-popular-heading">
                        <h2>Wisata Popular</h2>
                        <p>Lots of HIDDEN GEMS!</p>
                    </div>
                </div>

            </div>
        </section>
        <section class="section-popular-content" id="popularContent">
            <div class="container">
                <div class="section-popular-travel row justify-content-center">
                    @foreach($items as $item)
                    <div class="col-sm-6 col-md-4 col-lg-3">
                        <div class="card-travel text-center d-flex flex-column"
                            style="background-image: url('{{ $item->galleries->count() ? Storage::url($item->galleries->first()->image) : '' }}');">
                            <div class="travel-country">
                                {{$item->location}}
                            </div>
                            <div class="travel-location">{{$item->title}}</div>
                            <div class="travel-button mt-auto">
                                <a href="{{ route('detail', $item->slug) }}" class="btn btn-travel-details px4">
                                    View Details
                                </a>
                            </div>
                        </div>
                    </div>
                    @endforeach
                </div>
            </div>
        </section>

        <section class="section-networks" id="networks">
            <div class="container">
                <div class="row">
                    <div class="col-md-4">
                        <h2>Our Networks</h2>
                        <p>Companies are trusted us
                            <br>
                            more than just a tri
                        </p>
                    </div>
                    <div class="col-md-8 text-center">
                        <img src="frontend/images/partner.png" alt="Logo Partner" class="img-partner">
                    </div>
                </div>
            </div>
        </section>

        <section class="section-testimonial-heading" id="testimonialHeading">
            <div class="container">
                <div class="row">
                    <div class="col text-center">
                        <h2>
                            They Are Loving Us!
                        </h2>
                        <p>
                            Moments were giving them
                            <br>
                            the best experience
                        </p>
                    </div>
                </div>
            </div>

        </section>

        <section class="section-testimonial-content" id="testimonialContent">
            <div class="container">
                <div class="section-popular-travel row justify-content-center">
                    <div class="col-sm-6 col-md-6 col-lg-4">
                        <div class="card card-testimonial text-center">
                            <div class="testimonial-content">
                                <img src="frontend/images/testimonial-1.png" alt="User" class="mb-4 rounded-circle">
                                <h3 class="mb-4">Novrian H</h3>
                                <p class="testimonial">
                                    "Beautiful city
                                    with beautiful people,
                                    it was the best holiday ever!"
                                </p>
                            </div>
                            <hr>
                            <p class="trip-to mt-2">
                                Trip to Ubud Bali, Indonesia
                            </p>
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-6 col-lg-4">
                        <div class="card card-testimonial text-center">
                            <div class="testimonial-content">
                                <img src="frontend/images/testimonial-2.png" alt="User" class="mb-4 rounded-circle">
                                <h3 class="mb-4">Monica</h3>
                                <p class="testimonial">
                                    "Beyond my expectation!!"
                                </p>
                            </div>
                            <hr>
                            <p class="trip-to mt-2">
                                Trip to Raja Ampat, Indonesia
                            </p>
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-6 col-lg-4">
                        <div class="card card-testimonial text-center">
                            <div class="testimonial-content">
                                <img src="frontend/images/testimonial-3.png" alt="User" class="mb-4 rounded-circle">
                                <h3 class="mb-4">Rebecca</h3>
                                <p class="testimonial">
                                    "Komodo Are Amazing!
                                    I've never seen It before,
                                    hope i'll come back very soon"
                                </p>
                            </div>
                            <hr>
                            <p class="trip-to mt-2">
                                Trip to Komodo Island, Indonesia
                            </p>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12 text-center">
                        <a href="#" class="btn btn-need-help px-4 mt-4 mx-1">
                            I NEED HELP
                        </a>
                        <a href="{{ route('register')}}" class="btn btn-get-started px-4 mt-4 mx-1">
                            GET STARTED!
                        </a>
                    </div>
                </div>
            </div>
        </section>


    </main>
@endsection
